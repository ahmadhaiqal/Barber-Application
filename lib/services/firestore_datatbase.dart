...
class FirestoreDatabase{
  ...
  Stream<List<BarberShop>> getListBarberShop(){
    return _db.collection('barber shop').snapshots()
      .map((QuerySnapshot querySnapshot) =>
        querySnapshot.docs.map(
        (DocumentSnapshot documentSnapshot) => BarberShop(shopName: documentSnapshot.get('shopName'))).toList());
  }
  Stream<BarberShop> getBarberShop(String userIdToken) {
    return _db.collection('barber shop').doc(userIdToken).snapshots()
      .map((DocumentSnapshot documentSnapshot) =>
      BarberShop(shopName: documentSnapshot.get('shopName')));
  }
  /*
